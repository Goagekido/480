...
<p>We used a Random Forest model to predict whether China, Iran, or Russia would initiate future interstate conflict. Initially we thought a Neural Network (NN) model or Decision-Tree model would work best for this data. This is because we have a lot of variables that would include inputs, weights, and want predicted outcomes. The Decision-tree model worked better than the NN model because our inputs have very different features (some quantitative, some qualitative). However, we found the Random Forest model to work best as it </p>
